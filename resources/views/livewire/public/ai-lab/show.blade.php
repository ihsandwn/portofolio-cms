        ...
        <div class="space-y-6">
            ...
            <div class="bg-gradient-to-br from-indigo-900 to-slate-900 rounded-2xl p-6 border border-indigo-500/30">
                ...
                <div class="flex flex-col gap-3">
                    @if($project->url)
                    <a href="{{ $project->url }}" target="_blank" class="w-full text-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold transition">
                        Launch Demo
                    </a>
                    @endif
                    @if($project->repo_url)
                     <a href="{{ $project->repo_url }}" target="_blank" class="w-full text-center px-4 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg font-bold transition border border-slate-700">
                        View Code
                    </a>
                    @endif
                </div>

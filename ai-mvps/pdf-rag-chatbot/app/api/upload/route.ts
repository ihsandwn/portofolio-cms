...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(file instanceof File) || file.size === 0 || file.size > MAX_FILE_SIZE) {
      console.error('[UPLOAD ERROR] Invalid file object or size');
      return NextResponse.json(genericError, { status: 400 });
    }
    const buffer = Buffer.from(await file.arrayBuffer());
    if (!hasPdfMagicBytes(buffer)) {
      console.error('[UPLOAD ERROR] Missing PDF magic bytes');
      return NextResponse.json(genericError, { status: 400 });
    }
    const pages = await extractTextFromPDF(buffer);
    if (pages.length > MAX_PAGES) {
      console.error('[UPLOAD ERROR] Page count exceeded MAX_PAGES');
      return NextResponse.json(genericError, { status: 400 });
    }
    const cleanPages = pages.map(page => ({ ...page, text: sanitizeText(page.text) }));
    if (cleanPages.reduce((n, page) => n + page.text.length, 0) > MAX_TEXT_LENGTH) {
      console.error('[UPLOAD ERROR] Text length exceeded MAX_TEXT_LENGTH');
      return NextResponse.json(genericError, { status: 400 });
    }
    const chunks = chunkPages(cleanPages);
    if (!chunks.length) {
      console.error('[UPLOAD ERROR] No chunks generated from pages:', cleanPages);
      return NextResponse.json(genericError, { status: 400 });
    }

...
export default function Home() {
  ...
  const handleCaptionGenerated = (data: CaptionResponse, url: string) => {
    setResult(data);
    setImageUrl(url);
  };
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="text-center flex-1">
              <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-blue-900 via-blue-600 to-blue-400 bg-clip-text text-transparent mb-2">
                {language === 'en' ? 'Image Captioner' : 'Caption Gambar'}
              </h1>
              <p className="text-gray-600 dark:text-gray-400 text-lg">
                {language === 'en' ? 'AI-powered image analysis and categorization' : 'Analisis dan kategorisasi gambar dengan AI'}
              </p>
            </div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <section className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">{language === 'en' ? 'Upload Image' : 'Unggah Gambar'}</h2>
            </div>
          ...
          <section className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-200 dark:border-gray-700">
            <div className="flex items-center gap-2 mb-4">
              <Tag className="w-5 h-5 text-blue-900" aria-hidden="true" />
              <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">{language === 'en' ? 'Analysis Results' : 'Hasil Analisis'}</h2>
            </div>
            <CaptionResults result={result} imageUrl={imageUrl} language={language} />
          </section>
        </div>
